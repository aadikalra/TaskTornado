...
-- Board members policies (if they exist from previous migration attempts)
DROP POLICY IF EXISTS "Anyone can view board members" ON discussion_board_members;
DROP POLICY IF EXISTS "Users can join boards" ON discussion_board_members;
DROP POLICY IF EXISTS "Users can leave boards" ON discussion_board_members;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'discussion_boards_created_by_fkey'
  ) THEN
    ALTER TABLE discussion_boards
