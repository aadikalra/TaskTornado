...
export const RoleSwitcher = () => {
    ...
    return (
        <div
            ...
            style={{
                top: isSmall ? '16px' : '12px',
                right: isSmall ? '16px' : '12px',
            ...
            {roles.map((role) => (
                <button
                    ...
                    className={`relative px-4 py-1.5 font-bold rounded-xl transition-all duration-300 z-[1]
                        ${isSmall ? 'text-[12px]' : 'text-[11px]'}
                        ${activeView === role.key
                            ? 'text-[#275085] dark:text-[#4a9cdb]'
                            : 'text-zinc-500 dark:text-zinc-400 hover:text-[#275085] dark:hover:text-[#4a9cdb]'}`}
                >
                    {activeView === role.key && (
                        <motion.div
                            layoutId="activeRoleTabUniversal"
                            className="absolute inset-0 bg-[#fff8fa] dark:bg-zinc-800 rounded-xl z-[-1]"
