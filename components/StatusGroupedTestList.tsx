  ...
  const renderTestRow = useCallback((test: Test, index: number) => {
    ...
    return (
      ...
      >
        ...
        >
          <HugeIcon name={TypeIcon} size={14} className="w-3.5 h-3.5" style={{ color: color.header }} />
        </div>
        ...
        <div className="shrink-0 flex items-center gap-2">
          ...
          {/* Chevron */}
          <HugeIcon name="ArrowRight01" size={14} className="w-3.5 h-3.5 text-sky-300 dark:text-sky-600 opacity-0 group-hover:opacity-100 transition-opacity" />
        </div>
      </motion.div>
    );
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="flex-1 relative">
          <HugeIcon name="Search01" size={14} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-sky-400 dark:text-sky-500" />
          <Input
            type="text"
        ...
        <Select value={filter} onValueChange={(value: FilterOption) => setFilter(value)}>
          <SelectTrigger className="w-full sm:w-[140px] h-9 text-sm bg-white dark:bg-gray-900 border-sky-200 dark:border-gray-700 text-sky-900 dark:text-white hover:border-sky-500 rounded-xl">
            <HugeIcon name="Filter" size={14} className="w-3.5 h-3.5 mr-1.5 text-sky-400" />
            <SelectValue />
          </SelectTrigger>
        ...
        <>
          <div className="flex items-center gap-3 pt-1">
            ...
            >
              ...
              <HugeIcon name="ArrowDown01" size={12} className={`w-3 h-3 transition-transform duration-300 ${showTaken ? 'rotate-180' : ''}`} />
            </button>
            <div className="h-px bg-sky-100 dark:bg-gray-800 flex-1" />
          </div>
