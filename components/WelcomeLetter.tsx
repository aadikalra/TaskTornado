...
export default function WelcomeLetter({ isOpen, onClose }: WelcomeLetterProps) {
  const [isLetterOpen, setIsLetterOpen] = useState(false);

  // Reset letter state when modal opens and auto-open envelope
  React.useEffect(() => {
    if (isOpen) {
        ...
        <div className="envelope-container relative cursor-pointer" onClick={() => setIsLetterOpen(!isLetterOpen)}>
          ...
          >
            ...
            >
              ...
              {/* Top flap */}
              <div
                className={`flap-top absolute top-0 left-0 border-solid ${isLetterOpen ? 'open' : ''}`}
                style={{ width: 0, height: 0, zIndex: isLetterOpen ? 1 : 5 }}
              />

              {/* Wax seal */}
              <div
                className={`seal absolute flex items-center justify-center rounded-full ${isLetterOpen ? 'open' : ''}`}
                style={{
                  top: '240px',
                  left: '50%',
                  transform: isLetterOpen ? 'translate(-50%, -50%) scale(0.5)' : 'translate(-50%, -50%)',
                  width: '56px',
                  height: '56px',
                  background: 'radial-gradient(circle at 38% 35%, #3b82f6 0%, #2563eb 45%, #1e40af 100%)',
                  zIndex: 6,
                  boxShadow:
                ...
                {[...Array(14)].map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      height: '7px',
                      background:
                        'radial-gradient(circle at 40% 35%, #2563eb, #1e40af)',
                      top: '50%',
                      left: '50%',
              ...
              {/* Letter */}
              <div
                className={`letter absolute flex flex-col gap-4 overflow-hidden ${isLetterOpen ? 'open' : ''}`}
                style={{
                  bottom: 0,
                  left: '50%',
                  transform: isLetterOpen ? 'translate(-50%, -200px)' : 'translateX(-50%)',
                  width: '550px',
                  height: '420px',
                  backgroundColor: '#fefcf7',
                  zIndex: isLetterOpen ? 6 : 2,
                  padding: '35px 40px',
                  boxShadow: isLetterOpen
              ...
              {/* Subtle fold highlight at bottom flap's top edge */}
              <div
                className="absolute"
                style={{
                  bottom: '210px',
