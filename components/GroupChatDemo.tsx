        ...
        <div className={cn('flex flex-col bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 overflow-hidden', className)}>
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-zinc-800">
                <div className="flex items-center gap-2">
                    <div>
                        <p className="text-[11px] font-bold text-gray-900 dark:text-white leading-tight">AP Chemistry</p>
                        <p className="text-[9px] text-gray-400 dark:text-zinc-500 font-medium">4 members</p>
                    </div>
                </div>
                <div className="flex items-center gap-1.5 px-2 py-0.5 bg-green-50 dark:bg-green-950/20 rounded-full border border-green-100 dark:border-green-900/30">
                    <span className="text-[9px] font-bold text-green-600 dark:text-green-400 uppercase tracking-wider">Live</span>
                </div>
            </div>
                                ...
                                <div className={cn('max-w-[80%] flex flex-col gap-0.5', msg.isOwn && 'items-end')}>
                                    {showHeader && (
                                        <div className={cn('flex items-center gap-1.5 px-1', msg.isOwn && 'flex-row-reverse')}>
                                            <span className="text-[9px] font-bold text-gray-700 dark:text-gray-300">{msg.name}</span>
                                            <span className="text-[8px] text-gray-400 dark:text-zinc-600 font-medium">{msg.time}</span>
                                        </div>
                                    )}
                                    <div className={cn(
                                        'px-2.5 py-1.5 rounded-xl text-[11px] leading-relaxed w-fit font-medium',
                                        msg.isOwn
                                            ? 'bg-[#264f84] dark:bg-blue-600 text-white shadow-sm'
            ...
            <div className="px-2 py-2 border-t border-gray-100 dark:border-zinc-800 bg-gray-50/50 dark:bg-zinc-900/50">
                <div className="flex items-center gap-1.5 bg-white dark:bg-zinc-800 rounded-full px-4 py-1.5 border border-gray-200 dark:border-zinc-700">
                    <input
                        type="text"
                        value={inputValue}
                        onChange={(e) => setInputValue(e.target.value)}
                        placeholder="Type a message..."
                        className="flex-1 bg-transparent text-[11px] text-gray-700 dark:text-gray-300 placeholder:text-gray-400 dark:placeholder:text-zinc-600 outline-none font-medium"
                        readOnly
                    />
                </div>
            </div>
