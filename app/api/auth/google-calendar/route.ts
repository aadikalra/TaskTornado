...
export async function GET(request: NextRequest) {
  ...
  try {
    if (!googleIntegrationsEnabled('calendar')) {
    ...
    const oauth2Client = createGoogleOAuthClient('calendar');
    ...
    if (!tokens.access_token) return redirect(request, 'error');

    const { data: account } = await google
      .oauth2({ version: 'v2', auth: oauth2Client })
      .userinfo.get();

    if (!account.email) return redirect(request, 'error');

    await saveGoogleConnection(user.id, 'calendar', tokens, account);
