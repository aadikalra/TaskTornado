...
// ─── Main Component ─────────────────────────────────────────────────────────────
// ─── Known flag tokens (tab-separated columns between name and score) ────────
const FLAG_TOKENS = new Set([
    'collected', 'late', 'missing', 'exempt from final grade',
    'absent', 'incomplete', 'excluded from final grade',
]);

export default function PublicGradeCalculatorPage() {
    // Calculator State
    ...
    // ─── Smart Paste Parser ──────────────────────────────────────────────────────
    const parseGradesFromText = () => {
        setParseError('');
        setParsedCount(0);
        const text = rawText.trim();
        if (!text) { setParseError('Paste your grades data first.'); return; }

        const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
        const parsed: Assignment[] = [];

        for (const line of lines) {
            // Skip header, footer, and metadata lines
            if (/^Due\s?Date/i.test(line)) continue;
            if (/^Grade\s+stored/i.test(line)) continue;
            if (!/^\d{1,2}\/\d{1,2}\/\d{2,4}/.test(line)) continue;

            const cols = line.split('\t');
            if (cols.length < 4) continue;

            // Column 0 = date, Column 1 = category, Column 2 = assignment name
            const rawCategory = cols[1]?.trim().toLowerCase() || '';
            const name = cols[2]?.trim() || '';
            if (!name) continue;

            const category: 'practice' | 'assessment' =
                rawCategory.includes('assessment') || rawCategory.includes('test') || rawCategory.includes('exam')
                    ? 'assessment' : 'practice';

            // Find the score column by scanning from column 3 onward for X/Y pattern
            let pointsEarned = 0;
            ...
            for (let i = 3; i < cols.length; i++) {
                ...
                // Match score patterns like "95/100", "42.5/50", "--/11"
                const scoreMatch = cell.match(/^([\d.]+|--)\/([\d.]+)$/);
                if (scoreMatch) {
                    const earned = scoreMatch[1];
                    const possible = scoreMatch[2];
                    // Skip unscored entries ("--")
                    if (earned === '--') { foundScore = false; break; }
                    pointsEarned = parseFloat(earned) || 0;
                    pointsPossible = parseFloat(possible) || 0;
                    foundScore = true;
                    break;
                }
            }

            if (!foundScore || pointsPossible <= 0) continue;

            parsed.push({ name, category, pointsEarned, pointsPossible });
        }

        if (parsed.length === 0) {
            setParseError('No valid grade rows found. Make sure you copy-paste directly from your grade portal.');
            return;
        }
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8 sm:mb-10">
                    ...
                    >
                        ...
                        <p className="text-sm sm:text-base text-sky-600 dark:text-sky-300 font-medium">
                            Weighted grade calculator for any class — no account required.
                        </p>
                    </motion.div>
                            ...
                            {(['weights', 'paste', 'results'] as Step[]).map((s, i) => (
                                <React.Fragment key={s}>
                                    <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold transition-colors ${step === s
                                        ...
                                        <span>{i + 1}.</span>
                                        <span>{s === 'weights' ? 'Weights' : s === 'paste' ? 'Enter' : 'Results'}</span>
                                    </div>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1 p-0.5 bg-[#ebf6b5]/40 dark:bg-sky-500/10 rounded-full w-fit">
                                {(['paste', 'manual'] as const).map(mode => (
                                    ...
                                    >
                                        {mode === 'paste' ? 'Paste Grades' : 'Manual Entry'}
                                    </button>
                                ))}
                            </div>
                                ...
                                <div className="bg-[#f5f9fc] dark:bg-zinc-800 rounded-[24px] overflow-hidden shadow-sm">
                                    ...
                                    <div className="relative">
                                        <textarea
                                            ...
                                            placeholder={"Paste your grades table here...\n\nCopy all rows from your gradebook portal (e.g. Aeries, PowerSchool, Canvas).\nThe parser auto-detects columns like:\n\nDue Date  Category  Assignment  ...  Score  Grade\n05/22/2026  Assessment  Unit 8 Beta  ...  95/100  A"}
                                    ...
                                    {parseError && (
                                        <div className="mx-5 mb-4 flex items-start gap-2.5 px-4 py-3 bg-red-50 dark:bg-red-500/10 border border-red-200/60 dark:border-red-500/20 rounded-2xl">
                                            <HugeIcon name="AlertCircle" size={14} className="w-3.5 h-3.5 text-red-500 shrink-0 mt-0.5" />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-xs font-bold text-red-700 dark:text-red-300 mb-1">{parseError}</p>
                                                <button
                                                    onClick={() => { setEntryMode('manual'); setParseError(''); }}
                                                    className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-semibold text-sky-700 bg-[#ebf6b5] hover:bg-[#e0efa0] border border-[#d4e88e] rounded-lg transition-colors mt-1"
                                                >
                            ...
                            {entryMode === 'manual' && (
                                <>
                                    ...
                                    <div className="bg-[#f5f9fc] dark:bg-zinc-800 rounded-[24px] overflow-hidden shadow-sm">
                                        ...
                                        <div className="px-5 py-3 border-t border-sky-100 dark:border-sky-900/20 flex items-center justify-end">
                                            <button
