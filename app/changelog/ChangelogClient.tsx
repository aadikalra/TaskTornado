...
export default function ChangelogClient({ initialVersions }: ChangelogClientProps) {
  ...
  const getVisibleVersions = () => {
    if (!showArchived) {
      // Only show versions 2.0 and newer
      return versions.filter(v => compareVersions(v.version, '2.0') >= 0);
    }
    return versions;
  };
      ...
      <div className="relative z-10 w-full mx-auto px-4 sm:px-6 md:px-12 lg:px-16 pt-28 pb-16">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8 sm:mb-10">
          ...
          >
            ...
            >
              <HugeIcon name="Rotate01" className={`h-3.5 w-3.5 ${isRefreshing ? 'animate-spin' : ''}`} />
        ...
        {isOutdated && (
          ...
          >
            <div className="flex items-start gap-3">
              <HugeIcon name="AlertCircle" className="h-5 w-5 text-amber-600 dark:text-amber-400 mt-0.5 flex-shrink-0" />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="bg-[#f5f9fc] dark:bg-zinc-800 rounded-[24px] p-5 sm:p-6">
                  ...
                  <ul className="space-y-1.5 sm:space-y-2">
                    <li className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-sky-700/70 dark:text-sky-400/70">
                      ...
                      <span className="leading-relaxed">Forgot password functionality not working properly</span>
                    </li>
                  </ul>
                </div>
                ...
                <div className="bg-[#f5f9fc] dark:bg-zinc-800 rounded-[24px] p-5 sm:p-6">
                  ...
                  <ul className="space-y-1.5 sm:space-y-2">
                    <li className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-sky-700/70 dark:text-sky-400/70">
                      <span className="text-sky-500/40 dark:text-sky-400/40 mt-0.5 shrink-0">•</span>
                      <span className="leading-relaxed">Facebook sign-in integration for social login</span>
                    </li>
                    <li className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-sky-700/70 dark:text-sky-400/70">
                      <span className="text-sky-500/40 dark:text-sky-400/40 mt-0.5 shrink-0">•</span>
                      <span className="leading-relaxed">Apple sign-in integration for social login</span>
                    </li>
                    <li className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-sky-700/70 dark:text-sky-400/70">
                      <span className="text-sky-500/40 dark:text-sky-400/40 mt-0.5 shrink-0">•</span>
                      <span className="leading-relaxed">Forgot password functionality with email reset</span>
                    </li>
                    <li className="flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-sky-700/70 dark:text-sky-400/70">
                      <span className="text-sky-500/40 dark:text-sky-400/40 mt-0.5 shrink-0">•</span>
            ...
            <div className="space-y-8 sm:space-y-12">
              {loading ? (
                <div className="flex flex-col items-center justify-center py-16">
                  <HugeIcon name="LoaderPinwheel" className="h-8 w-8 animate-spin text-sky-500/50 mb-4" />
                  <p className="text-sky-700/60 dark:text-sky-400/60">Loading changelog...</p>
                </div>
              ) : error ? (
                <div className="flex flex-col items-center justify-center py-16">
                  <HugeIcon name="AlertCircle" className="h-8 w-8 text-red-500 mb-4" />
                        ...
                        <div className={`relative flex flex-col sm:flex-row sm:items-start justify-between mb-4 pb-4 border-b ${isFuture ? 'border-sky-200 dark:border-sky-900/30' : 'border-sky-100 dark:border-sky-900/20'} gap-3`}>
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 text-xs sm:text-sm text-sky-700 dark:text-sky-300">
                              <HugeIcon name="Calendar02" className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                          ...
                          >
                            {open === v.version ? (
                              <>
                                Hide
                                <HugeIcon name="ArrowUp02" className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                              </>
                            ) : (
                              <>
                                Details
                                <HugeIcon name="ArrowDown01" className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                              </>
                            )}
                          </button>
                        </div>
            ...
            {!showArchived && (
              ...
              >
                <button
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3 bg-[#f5f9fc] dark:bg-zinc-800 border border-sky-200/60 dark:border-sky-800/30 rounded-full text-sm font-bold text-sky-600 dark:text-sky-400 hover:bg-[#ebf6b5]/60 dark:hover:bg-sky-500/20 transition-all active:scale-95"
                >
                  <HugeIcon name="ArrowRight01" className="w-4 h-4" />
                  Show Archived Updates
                </button>
                <p className="mt-3 text-xs sm:text-sm text-sky-600/50 dark:text-sky-400/50 max-w-md mx-auto">
                  View updates from versions before 2.0
                </p>
              </motion.div>
            )}
              ...
              {showArchived && (
                ...
                >
                  <div className="mt-8 sm:mt-12">
                    <div className="mb-6 pb-4 border-b border-sky-100 dark:border-sky-900/20">
                      <h3 className="text-lg font-bold text-sky-800 dark:text-sky-200 flex items-center gap-2">
                        <span>Archived Updates</span>
                        <span className="px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest bg-sky-100/60 dark:bg-sky-500/10 text-sky-600/60 dark:text-sky-400/60 rounded-full">
                          Before v2.0
                        </span>
                      </h3>
                      <p className="text-sm text-sky-600/60 dark:text-sky-400/60 mt-1">
                        Historical updates from early development versions
                      </p>
                    </div>
                    <div className="space-y-8 sm:space-y-12">
                      {filteredVersions
                        .filter(v => compareVersions(v.version, '2.0') < 0)
                        .map((v, i) => {
                          const today = new Date();
                          today.setHours(0, 0, 0, 0);
                          const versionDate = new Date(v.date + 'T00:00:00');
                          ...
                          return (
                            <div key={`archived-${v.version}`} id={`version-${v.version}`} className="relative scroll-mt-24 opacity-60">
                              <motion.div
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ delay: i * 0.05 }}
                              ...
                              >
                                {/* Version Header */}
                                <div className="relative flex flex-col sm:flex-row sm:items-start justify-between mb-4 pb-4 border-b border-sky-100 dark:border-sky-900/20 gap-3">
                                  <div className="flex-1">
                                    <div className="flex flex-wrap items-baseline gap-2 sm:gap-3 mb-2">
                                      <h2 className="text-lg sm:text-xl font-bold text-sky-700 dark:text-sky-300 leading-tight">
                                        {v.title}
                                      </h2>
                                      <span className="text-xs sm:text-sm font-mono text-sky-600 dark:text-sky-400">
                                        v{v.version}
                                      </span>
                                      <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest bg-gray-100/60 dark:bg-gray-500/20 text-gray-600 dark:text-gray-400 rounded-full">
                                        Archived
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-2 text-xs sm:text-sm text-sky-600 dark:text-sky-400">
                                      <HugeIcon name="Calendar02" className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                                      {new Date(v.date + 'T00:00:00').toLocaleDateString('en-US', {
                                        month: 'short',
                                        day: 'numeric',
                                ...
                                <ul className="space-y-1.5 sm:space-y-2">
                                  {v.short.map((txt, idx) => (
                                    <li key={idx} className="relative flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-sky-700 dark:text-sky-400">
                                      <span className="text-sky-500/60 dark:text-sky-400/60 mt-0.5 shrink-0">•</span>
                                      <span className="leading-relaxed wrap-break-word">{txt}</span>
                                    </li>
                                  ))}
                                      ...
                                      <ul className="mt-4 sm:mt-6 pt-4 sm:pt-6 space-y-1.5 sm:space-y-2 border-t border-sky-100 dark:border-sky-900/20">
                                        {v.full.map((txt, idx) => (
                                          <motion.li
                                            ...
                                            animate={{ opacity: 1, x: 0 }}
                                            transition={{ delay: idx * 0.02 }}
                                            className="relative flex items-start gap-2 sm:gap-3 text-xs sm:text-sm text-sky-600 dark:text-sky-400"
                                          >
                                            <span className="text-sky-500/60 dark:text-sky-400/60 mt-0.5 shrink-0">•</span>
                                            <span className="leading-relaxed wrap-break-word">{txt}</span>
                                          </motion.li>
            ...
            <div className="sticky top-24 space-y-6">
              <div>
                <h3 className="text-sm font-bold text-sky-500 dark:text-sky-400 mb-4 flex items-center gap-2">
                  <HugeIcon name="Hashtag" className="w-4 h-4" />
                  Version History
                </h3>
                <div
                ...
                >
                  {filteredVersions.map((v) => (
                    ...
                    >
                      ...
                      <HugeIcon name="ArrowRight01" className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </button>
                  ))}
                </div>
              </div>
